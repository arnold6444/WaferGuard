...
export default function FabOverview({ onNavigate }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!response.ok) throw new Error("Fab overview API를 확인해 주세요.");
      setOverview(await response.json());
      setError("");
    } catch (nextError) {
      setError(nextError.message || "Fab overview를 불러오지 못했습니다.");
    }
  }, []);
  ...
  if (!overview) return <div className="panel process-empty"><Icon name="gauge" size={22} /><strong>{error || "Fab 상태를 불러오는 중입니다…"}</strong><button type="button" className="btn" onClick={load}>다시 시도</button></div>;
    ...
    <div className="fab-overview-page">
      <section className="panel fab-overview-hero">
        <div>
          ...
          <h2>공정 설비와 Wafer 품질 이상을 한 화면에서 연결합니다.</h2>
          <p>{overview.disclaimer}</p>
        </div>
        <div className="fab-overview-health"><span className={`status-dot status-${overview.wafer_quality.status}`} /><div><strong>Wafer Quality</strong><span>{overview.wafer_quality.latest_wafer}</span><SourceTag value={overview.wafer_quality.data_source} /></div></div>
      </section>

      <div className="fab-overview-metrics">
        <OverviewMetric label="Active Lots" metric={overview.metrics.active_lots} icon="layers" />
        <OverviewMetric label="Running Equipment" metric={overview.metrics.running_equipment} icon="cpu" />
        <OverviewMetric label="Warnings" metric={overview.metrics.warnings} icon="alert" />
        <OverviewMetric label="Critical" metric={overview.metrics.critical} icon="pulse" />
      </div>

      <div className="fab-overview-grid">
        <Panel title="Process Status" icon="activity" right={<span className="chip">8 PROFILES</span>}>
        ...
        <Panel title="Recent Alerts" icon="alert" right={<span className="chip">TEMPORAL CANDIDATES</span>}>
      ...
      <section className="panel fab-flow-card">
        <div><span className="label-cap">권장 조사 흐름</span><strong>Etch warning → related Lot → Wafer trend → evidence → Agent review</strong></div>
        <button type="button" className="btn btn-accent" onClick={() => onNavigate?.({ id: "process", processId: "etch", tab: "anomaly" })}><Icon name="activity" size={14} />Etch anomaly 확인</button>
      </section>
    </div>
  );
}
